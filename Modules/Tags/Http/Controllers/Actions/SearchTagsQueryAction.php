    ...
    public function execute(User $auth_user, Request $request)
    {
        ...
        if ($request->input('created_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $tags = $tags->where('created_at', '>=', $from)->where('created_at', '<=', $to);
                } elseif ($from) {
                    $tags = $tags->where('created_at', '>=', $from);
                } elseif ($to) {
                    $tags = $tags->where('created_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
        ...
        if ($request->input('last_updated_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $tags = $tags->where('updated_at', '>=', $from)->where('updated_at', '<=', $to);
                } elseif ($from) {
                    $tags = $tags->where('updated_at', '>=', $from);
                } elseif ($to) {
                    $tags = $tags->where('updated_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
