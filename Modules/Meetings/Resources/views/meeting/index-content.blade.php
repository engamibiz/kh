        ...
        <div class="kt-content  kt-grid__item kt-grid__item--fluid" id="kt_content">
            ...
            <div class="kt-portlet kt-portlet--mobile">
                <div class="kt-portlet__head kt-portlet__head--lg">
                    <div class="kt-portlet__head-label">
                        ...
                        <h3 class="kt-portlet__head-title">
                            <span data-8xloadtitle>{{trans('meetings::meeting.meetings')}}</span>
                            <small>{{trans('meetings::meeting.list')}}</small>
                        </h3>
                    </div>
                    <div class="kt-portlet__head-toolbar">
                        <div class="kt-portlet__head-wrapper">
                            {{--
                            <!-- Full creation form -->
                            <a href="{{route('meetings.create')}}" data-8xload class="btn btn-brand btn-icon-sm">
                                <i class="flaticon2-plus"></i> {{trans('meetings::meeting.create_new')}}
                            </a>
                            --}}
                            <!-- @if (auth()->user()->hasPermission('create-meeting'))
                                <a href="{{route('meetings.create')}}" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#vcxl_modal" data-path="{{route('meetings.modals.create')}}" data-title="{{trans('meetings::meeting.create_meeting')}}" data-modal-load>
                                    <span>
                                        <i class="la la-plus"></i>
                                        <span>{{trans('meetings::meeting.create_meeting')}}</span>
                                    </span>
                                </a>
                            @endif -->
                            {{--
                ...
                <div class="kt-portlet__body">
                    ...
                    <!--begin: Datatable -->
                    <table class="table table-striped- table-bordered table-hover table-checkable datatable" id="meetings_table">
                        <thead>
                            <tr>
                                <th>{{__('meetings::meeting.id')}}</th>
                                <th>{{__('meetings::meeting.meeting')}}</th>
                                <th>{{__('meetings::meeting.user')}}</th>
                                <th>{{__('meetings::meeting.created_at')}}</th>
                                <th>{{__('meetings::meeting.last_updated_at')}}</th>
                                <th>{{__('meetings::meeting.actions')}}</th>
                            </tr>
                        </thead>
                    </table>
                    <!--end: Datatable -->
