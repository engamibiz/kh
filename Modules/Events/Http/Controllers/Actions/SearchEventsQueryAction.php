    ...
    public function execute(User $auth_user, Request $request)
    {
        ...
        if ($request->input('created_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $events = $events->where('created_at', '>=', $from)->where('created_at', '<=', $to);
                } elseif ($from) {
                    $events = $events->where('created_at', '>=', $from);
                } elseif ($to) {
                    $events = $events->where('created_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
        ...
        if ($request->input('last_updated_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $events = $events->where('updated_at', '>=', $from)->where('updated_at', '<=', $to);
                } elseif ($from) {
                    $events = $events->where('updated_at', '>=', $from);
                } elseif ($to) {
                    $events = $events->where('updated_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
