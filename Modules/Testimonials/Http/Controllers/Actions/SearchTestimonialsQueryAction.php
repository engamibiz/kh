...
class SearchTestimonialsQueryAction
{
    public function execute(User $auth_user, Request $request)
    {
        // Search testimonial
        $testimonials = (new Testimonial)->newQuery();
        ...
        if ($request->input('created_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $testimonials = $testimonials->where('created_at', '>=', $from)->where('created_at', '<=', $to);
                } elseif ($from) {
                    $testimonials = $testimonials->where('created_at', '>=', $from);
                } elseif ($to) {
                    $testimonials = $testimonials->where('created_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
        ...
        if ($request->input('last_updated_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $testimonials = $testimonials->where('updated_at', '>=', $from)->where('updated_at', '<=', $to);
                } elseif ($from) {
                    $testimonials = $testimonials->where('updated_at', '>=', $from);
                } elseif ($to) {
                    $testimonials = $testimonials->where('updated_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
