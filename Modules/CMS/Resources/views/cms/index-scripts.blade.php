                    ...
                    render: function(data, type, full, meta) {
                        ...
                        @if (auth()->user()->hasPermission('update-terms-condition') || auth()->user()->hasPermission('delete-terms-condition'))
                            ...
                            @if (auth()->user()->hasPermission('update-terms-condition'))
                                value += `
                                    <a href="{{route('terms.modals.update')}}" class="dropdown-item" data-toggle="modal" data-target="#fast_modal" data-path="{{route('terms.modals.update')}}" data-title="{{trans('cms::cms.update_term')}}" data-id="`+full.id+`" data-modal-load>
                                        <span>
                            ...
                            @if (auth()->user()->hasPermission('delete-terms-condition'))
                                value += `
                                    <a class="dropdown-item" data-8x-delete-it='{"container":false, "path":"`+delete_url+`", "callback": "deleteAboutCallback"}' href="#"><i class="la la-trash"></i> {{__('cms::cms.delete_term')}}</a>
                                `;
                            @endif
                            value += `
                                </div>
