                    ...
                    render: function(data, type, full, meta) {
                        ...
                        @if (auth()->user()->hasPermission('update-subscribe-mail') || auth()->user()->hasPermission('delete-subscribe-mail'))
                            ...
                            @if (auth()->user()->hasPermission('update-subscribe-mail'))
                                value += `
                                    <a href="{{route('contact_us.subscribes.modals.update')}}" class="dropdown-item" data-toggle="modal" data-target="#fast_modal" data-path="{{route('contact_us.subscribes.modals.update')}}" data-title="{{trans('contactus::contact_us.update_subscribe')}}" data-id="`+full.id+`" data-modal-load>
                                        <span>
                            ...
                            @if (auth()->user()->hasPermission('delete-subscribe-mail'))
                                value += `
                                    <a class="dropdown-item" data-8x-delete-it='{"container":false, "path":"`+delete_url+`", "callback": "deleteSubscribeCallback"}' href="#"><i class="la la-trash"></i> {{__('contactus::contact_us.delete_subscribe')}}</a>
                                `;
                            @endif
                            value += `
                                </div>
