...
@section('content')
    <!--begin::Form-->
    <form action="{{route('inventory.purposes.update')}}" method="POST" id="update_purpose_form" class="m-form m-form--fit m-form--label-align-right m-form--group-seperator" data-async data-callback="updatePurposeCallback" data-parsley-validate>
        @csrf
        <input type="hidden" name="id" id="id" value="{{$i_purpose->id}}" />
        <div class="m-portlet__body">
            <div class="form-group row">
                <div class="col-6">
                     <label for="order">{{__('inventory::inventory.order')}}</label>
                    <input name="order" id="order" type="text" class="form-control" placeholder="{{__('inventory::inventory.order')}}"  data-parsley-trigger="change focusout" data-parsley-type="integer" data-parsley-min="0" value="{{$i_purpose->order}}">
                </div>
                <div class="col-lg-4">
                ...
                <img src="{{asset('storage/'.$i_purpose->image)}}" class="w-100" alt="">
            </div>
            </div>
            <div class="form-group row">
                <div class="col-12 repeater">
                    <div data-repeater-list="translations">
                        @foreach ($i_purpose->translations as $translation)
                        ...
                        @endforeach
                        @if (!$i_purpose->translations->count())
                            <div data-repeater-item class="row">
                                <div class="col-6">
                                    {{-- <label for="language_id">{{__('inventory::inventory.language')}}</label> --}}
                                    <select class="form-control" id="language_id" name="language_id" required data-parsley-required data-parsley-required-message="{{__('inventory::inventory.please_select_the_language')}}" data-parsley-trigger="change focusout">
        ...
        <div class="m-portlet__foot m-portlet__no-border m-portlet__foot--fit">
            <div class="m-form__actions m-form__actions--solid">
                <div class="row">
                    <div class="col-lg-6">
                        <button type="submit" class="btn btn-success btn-brand">{{trans('inventory::inventory.update_purpose')}}</button>
                    </div>
                </div>
            </div>
        </div>
...
<!-- Callback function -->
<script>
    function updatePurposeCallback() {
        // Close modal
