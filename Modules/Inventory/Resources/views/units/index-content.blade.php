                ...
                <div class="kt-portlet__head head-zindex-fix">
                    <div class="kt-portlet__head-label">
                        <h3 class="kt-portlet__head-title kt-font-danger animated bounceIn">
                            <i class="fas fa-city"></i> <span>{{__('inventory::inventory.units')}}</span> <small><span
                                        class="kt-font-success kt-font-bolder">{{trans('inventory::inventory.real_estate')}}</span></small>
                        </h3>
                    </div>
                    <div class="kt-portlet__head-toolbar">
                        ...
                        <ul class="nav nav-pills nav-pills-sm animated fadeIn" id="lccatypesFilter" role="tablist">
                            <li class="nav-item">
                                <a class="nav-link active filter-lcca-type"
                                   data-push-it='{"target":"#workspaceTitle", "type":"html", "html": "<i class=\"fas fa-building fa-sm\"></i> {{__("inventory::inventory.properties")}}"}'
                                   data-toggle="tab" href="#" role="tab">
                                    <i class="fas fa-building"></i> {{trans('inventory::inventory.units')}}
                                </a>
                            </li>
                            <li class="nav-item">
                                <button class="nav-link filter-lcca-type border-0" id="deleteBulkUnits">
                            ...
                            <li class="nav-item dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right" x-placement="bottom-end"
                                     style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(95px, 59px, 0px);">
                                    @haspermission('create-inventory-unit')
                                    <a class="dropdown-item" href="{{route('inventory.units.create')}}"><i
                                                class="fas fa-building kt-padding-t-5 fa-sm kt-menu__link-bullet"></i> {{trans('inventory::inventory.unit')}}
                                    </a>
                                    @endhaspermission

                                    @haspermission('create-inventory-project')
                                    <a class="dropdown-item" href="{{route('inventory.projects.create')}}"><i
                                                class="fas fa-city kt-padding-t-5 fa-sm kt-menu__link-bullet"></i> {{trans('inventory::inventory.project')}}
                                    </a>
                                    @endhaspermission
                                </div>
                            </li>
                            ...
                            auth()->user()->hasPermission('index-inventory-design-types')
                                ...
                                <li class="nav-item">
                                    <a class="nav-link" href="{{route('inventory.settings')}}" data-skin="dark"
                                       data-toggle="kt-tooltip" data-placement="top" title=""
                                       data-original-title="{{trans('inventory::inventory.settings')}}" role="tab">
                        ...
                        <div class="col-12 col-md-12 kt-margin-b-15">
                            <div id="BIListing">
                                ...
                                <table class="table table-striped table-bordered table-hover table-checkable datatable"
                                    ...
                                    <tbody>
                                    @foreach($i_units as $i_unit)
                                        <tr>
                                            <td></td>
                                            <td>{{$i_unit->id}}</td>
                                            <td>{{$i_unit->title}}</td>
                                            <td>{{$i_unit->area}}</td>
                                            <td>{{$i_unit->offeringType['value']}}</td>
                                            <td>
                                                @if (!empty($i_unit->country['value']))
                                                    {{$i_unit->country['value']}}-
                                                    @if(!empty($i_unit->region['value']))
                                                        {{$i_unit->region['value']}}-
                                                        @if(!empty($i_unit->city['value']))
                                                            {{$i_unit->city['value']}}
                                                        @endif
                                                    @endif
                                            ...
                                            <td>{{$i_unit->price}}</td>
                                            <td>{{
                                                strlen($i_unit->description) > 50
                                                    ? substr($i_unit->description, 0, 50) . '...'
                                                    : $i_unit->description
                                            }}</td>
                                            <td>{{ $i_unit->created_at->format('d-m-Y H:i:s') }}</td>
                                            <td>{{ $i_unit->updated_at->format('d-m-Y H:i:s') }}</td>
                                            <td>
{{--                                                {{--}}
{{--                                                $i_unit->created_by--}}
{{--                                                    ? $i_unit->createdBy->name--}}
{{--                                                    : '-'--}}
{{--                                            }}--}}
                                            </td>
                                            <td>{{__('inventory::inventory.last_updated_by')}}</td>
                                            <td></td>
                                        </tr>
                                    @endforeach
                                    </tbody>
