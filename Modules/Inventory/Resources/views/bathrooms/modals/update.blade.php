...
@section('content')
    <!--begin::Form-->
    <form action="{{route('inventory.bathrooms.update')}}" method="POST" id="update_bathroom_form" class="m-form m-form--fit m-form--label-align-right m-form--group-seperator" data-async data-callback="updateBathroomCallback" data-parsley-validate>
        @csrf
        <input type="hidden" name="id" id="id" value="{{$i_bathroom->id}}" />
        <div class="m-portlet__body">
            <div class="form-group row">
                <div class="col-6">
                    {{-- <label for="order">{{__('inventory::inventory.order')}}</label> --}}
                    <input name="order" id="order" type="text" class="form-control" placeholder="{{__('inventory::inventory.order')}}"  data-parsley-trigger="change focusout" data-parsley-type="integer" data-parsley-min="0" value="{{$i_bathroom->order}}">
                </div>
            </div>
            <div class="form-group row">
                <div class="col-6">
                    {{-- <label for="count">{{__('inventory::inventory.count')}}</label> --}}
                    <input name="count" id="count" type="text" class="form-control" placeholder="{{__('inventory::inventory.count')}}" required data-parsley-required data-parsley-required-message="{{__('inventory::inventory.count_is_required')}}" data-parsley-trigger="change focusout" data-parsley-type="integer" data-parsley-min="0" value="{{$i_bathroom->count}}">
                </div>
            </div>
            <div class="form-group row">
                <div class="col-12 repeater">
                    <div data-repeater-list="translations">
                        @foreach ($i_bathroom->translations as $translation)
                            <div data-repeater-item class="row">
                                ...
                                <div class="col-6">
                                    ...
                                    <input name="displayed_text" id="displayed_text" type="text" class="form-control" placeholder="{{__('inventory::inventory.please_enter_the_bathroom')}}" required data-parsley-required data-parsley-required-message="{{__('inventory::inventory.please_enter_the_bathroom')}}" data-parsley-trigger="change focusout" data-parsley-maxlength="150" data-parsley-maxlength-message="{{__('inventory::inventory.bathroom_max_is_150_characters_long')}}" value="{{$translation->displayed_text}}">
                                </div>
                        ...
                        @endforeach
                        @if (!$i_bathroom->translations->count())
                            <div data-repeater-item class="row">
                                <div class="col-6">
                                    {{-- <label for="language_id">{{__('inventory::inventory.language')}}</label> --}}
                                    <select class="form-control" id="language_id" name="language_id" required data-parsley-required data-parsley-required-message="{{__('inventory::inventory.please_select_the_language')}}" data-parsley-trigger="change focusout">
        ...
        <div class="m-portlet__foot m-portlet__no-border m-portlet__foot--fit">
            <div class="m-form__actions m-form__actions--solid">
                <div class="row">
                    <div class="col-lg-6">
                        <button type="submit" class="btn btn-success btn-brand">{{trans('inventory::inventory.update_bathroom')}}</button>
                    </div>
                </div>
            </div>
        </div>
...
<!-- Callback function -->
<script>
    function updateBathroomCallback() {
        // Close modal
        $('#fast_modal').modal('toggle');
        // Reload datatable
        bathrooms_table.ajax.reload(null, false);
    }
</script>
