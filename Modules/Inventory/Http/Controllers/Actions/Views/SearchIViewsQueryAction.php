...
class SearchIViewsQueryAction
{
    public function execute(User $auth_user, Request $request)
    {
        // Get the i_views
        $i_views = (new IView)->newQuery();
        ...
        if ($request->input('created_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $i_views = $i_views->where('created_at', '>=', $from)->where('created_at', '<=', $to);
                } elseif ($from) {
                    $i_views = $i_views->where('created_at', '>=', $from);
                } elseif ($to) {
                    $i_views = $i_views->where('created_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
        ...
        if ($request->input('last_updated_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $i_views = $i_views->where('updated_at', '>=', $from)->where('updated_at', '<=', $to);
                } elseif ($from) {
                    $i_views = $i_views->where('updated_at', '>=', $from);
                } elseif ($to) {
                    $i_views = $i_views->where('updated_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
