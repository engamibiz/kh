...
class SearchIOfferingTypesQueryAction
{
    public function execute(User $auth_user, Request $request)
    {
        // Get the i_offering_types
        $i_offering_types = (new IOfferingType)->newQuery();
        ...
        if ($request->input('created_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $i_offering_types = $i_offering_types->where('created_at', '>=', $from)->where('created_at', '<=', $to);
                } elseif ($from) {
                    $i_offering_types = $i_offering_types->where('created_at', '>=', $from);
                } elseif ($to) {
                    $i_offering_types = $i_offering_types->where('created_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
        ...
        if ($request->input('last_updated_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $i_offering_types = $i_offering_types->where('updated_at', '>=', $from)->where('updated_at', '<=', $to);
                } elseif ($from) {
                    $i_offering_types = $i_offering_types->where('updated_at', '>=', $from);
                } elseif ($to) {
                    $i_offering_types = $i_offering_types->where('updated_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
