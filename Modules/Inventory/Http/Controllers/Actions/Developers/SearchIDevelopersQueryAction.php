...
class SearchIDevelopersQueryAction
{

    public function execute(User $auth_user, Request $request)
    {
        // Get the i_developers
        $i_developers = (new IDeveloper)->newQuery();
        ...
        if ($request->input('created_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $i_developers = $i_developers->where('created_at', '>=', $from)->where('created_at', '<=', $to);
                } elseif ($from) {
                    $i_developers = $i_developers->where('created_at', '>=', $from);
                } elseif ($to) {
                    $i_developers = $i_developers->where('created_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
        ...
        if ($request->input('last_updated_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $i_developers = $i_developers->where('updated_at', '>=', $from)->where('updated_at', '<=', $to);
                } elseif ($from) {
                    $i_developers = $i_developers->where('updated_at', '>=', $from);
                } elseif ($to) {
                    $i_developers = $i_developers->where('updated_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
        }
        $i_developers->with('translations');
        return $i_developers;
    }
}