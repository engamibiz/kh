...
class SearchIFurnishingStatusesQueryAction
{
    public function execute(User $auth_user, Request $request)
    {
        // Get the i_furnishing_statuses
        $i_furnishing_statuses = (new IFurnishingStatus)->newQuery();
        ...
        if ($request->input('created_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $i_furnishing_statuses = $i_furnishing_statuses->where('created_at', '>=', $from)->where('created_at', '<=', $to);
                } elseif ($from) {
                    $i_furnishing_statuses = $i_furnishing_statuses->where('created_at', '>=', $from);
                } elseif ($to) {
                    $i_furnishing_statuses = $i_furnishing_statuses->where('created_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
        ...
        if ($request->input('last_updated_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $i_furnishing_statuses = $i_furnishing_statuses->where('updated_at', '>=', $from)->where('updated_at', '<=', $to);
                } elseif ($from) {
                    $i_furnishing_statuses = $i_furnishing_statuses->where('updated_at', '>=', $from);
                } elseif ($to) {
                    $i_furnishing_statuses = $i_furnishing_statuses->where('updated_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
