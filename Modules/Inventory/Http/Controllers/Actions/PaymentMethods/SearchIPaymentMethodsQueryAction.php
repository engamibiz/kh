...
class SearchIPaymentMethodsQueryAction
{
    public function execute(User $auth_user, Request $request)
    {
        // Get the i_payment_methods
        $i_payment_methods = (new IPaymentMethod)->newQuery();
        ...
        if ($request->input('created_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $i_payment_methods = $i_payment_methods->where('created_at', '>=', $from)->where('created_at', '<=', $to);
                } elseif ($from) {
                    $i_payment_methods = $i_payment_methods->where('created_at', '>=', $from);
                } elseif ($to) {
                    $i_payment_methods = $i_payment_methods->where('created_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
        ...
        if ($request->input('last_updated_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $i_payment_methods = $i_payment_methods->where('updated_at', '>=', $from)->where('updated_at', '<=', $to);
                } elseif ($from) {
                    $i_payment_methods = $i_payment_methods->where('updated_at', '>=', $from);
                } elseif ($to) {
                    $i_payment_methods = $i_payment_methods->where('updated_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
