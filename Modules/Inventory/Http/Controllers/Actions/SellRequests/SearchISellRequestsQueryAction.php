...
class SearchISellRequestsQueryAction
{

    public function execute(User $auth_user, Request $request)
    {
        // Get the Sell requests
        $i_sell_requests = (new ISellRequest())->newQuery();
        ...
        if ($request->input('created_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $i_sell_requests = $i_sell_requests->where('created_at', '>=', $from)->where('created_at', '<=', $to);
                } elseif ($from) {
                    $i_sell_requests = $i_sell_requests->where('created_at', '>=', $from);
                } elseif ($to) {
                    $i_sell_requests = $i_sell_requests->where('created_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
        ...
        if ($request->input('last_updated_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $i_sell_requests = $i_sell_requests->where('updated_at', '>=', $from)->where('updated_at', '<=', $to);
                } elseif ($from) {
                    $i_sell_requests = $i_sell_requests->where('updated_at', '>=', $from);
                } elseif ($to) {
                    $i_sell_requests = $i_sell_requests->where('updated_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
        }
        return $i_sell_requests;
    }
}