...
class SearchIPurposesQueryAction
{
    public function execute(User $auth_user, Request $request)
    {
        // Get the i_purposes
        $i_purposes = (new IPurpose)->newQuery();
        ...
        if ($request->input('created_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $i_purposes = $i_purposes->where('created_at', '>=', $from)->where('created_at', '<=', $to);
                } elseif ($from) {
                    $i_purposes = $i_purposes->where('created_at', '>=', $from);
                } elseif ($to) {
                    $i_purposes = $i_purposes->where('created_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
        ...
        if ($request->input('last_updated_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $i_purposes = $i_purposes->where('updated_at', '>=', $from)->where('updated_at', '<=', $to);
                } elseif ($from) {
                    $i_purposes = $i_purposes->where('updated_at', '>=', $from);
                } elseif ($to) {
                    $i_purposes = $i_purposes->where('updated_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
