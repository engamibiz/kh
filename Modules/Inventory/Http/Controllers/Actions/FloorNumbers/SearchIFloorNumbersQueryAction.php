...
class SearchIFloorNumbersQueryAction
{
    public function execute(User $auth_user, Request $request)
    {
        // Get the i_floor_numbers
        $i_floor_numbers = (new IFloorNumber)->newQuery();
        ...
        if ($request->input('created_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $i_floor_numbers = $i_floor_numbers->where('created_at', '>=', $from)->where('created_at', '<=', $to);
                } elseif ($from) {
                    $i_floor_numbers = $i_floor_numbers->where('created_at', '>=', $from);
                } elseif ($to) {
                    $i_floor_numbers = $i_floor_numbers->where('created_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
        ...
        if ($request->input('last_updated_at_range')) {
            try {
                ...
                if ($from && $to) {
                    $i_floor_numbers = $i_floor_numbers->where('updated_at', '>=', $from)->where('updated_at', '<=', $to);
                } elseif ($from) {
                    $i_floor_numbers = $i_floor_numbers->where('updated_at', '>=', $from);
                } elseif ($to) {
                    $i_floor_numbers = $i_floor_numbers->where('updated_at', '<=', $to);
                }
            } catch (\Exception $e) {
                //
            }
